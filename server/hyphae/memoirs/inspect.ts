import type { ConceptLinkRow, ConceptRow } from '../../types.ts'
...
export function memoirInspect(
  ...
  const neighborLinks: Array<{ targetId: string; link: ConceptLinkRow; direction: 'outgoing' | 'incoming' }> = []
  ...
  if (conceptIdsToFetch.size > 0) {
    ...
    for (const neighborLink of neighborLinks) {
      const linkedConcept = conceptMap.get(neighborLink.targetId)
      if (linkedConcept) {
        neighbors.push({
